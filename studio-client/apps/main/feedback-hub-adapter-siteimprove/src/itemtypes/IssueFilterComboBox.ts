...
class IssueFilterComboBox extends IssueFilterComboBoxBase{
  ...
  ...ConfigUtils.append({plugins: [
    Config(BindListPlugin, { bindTo: this.getFilterOptionsExpression(), sortField: "code",
      fields:[
        Config(DataField, { name: "value", encode: false}),
        Config(DataField, { name: "description", encode: false})
      ]
    }),
    Config(BindPropertyPlugin, { bindTo: config.bindTo, bidirectional: true, ifUndefined: IssueFilterComboBoxBase.VALUE_ALL, componentEvent: "change"})
  ]})
}),config))());
  }}
