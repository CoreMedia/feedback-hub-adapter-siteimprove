...
interface ComparingGaugeItemPanelConfig extends Config<ComparingGaugeItemPanelBase> {
  ...
  items:[
    Config(Container, { flex: 1,
      items:[
        Config(GaugeFeedbackItemPanel, { flex: 1,
    ...
    Config(Container, {
      items:[
        Config(IconDisplayField, { iconCls:  this.getDiffIconCls(config),
                             ui:  this.getIconUI(config),
                             scale: "medium",
                             margin: "0 6 0 0"}),
        Config(IconDisplayField, { value: this.getDiffLabel(config), overflowBehaviour: OverflowBehaviour.ELLIPSIS})
      ],
      plugins:[
